...
namespace Application.Producto.Validators
{
    public class ActualizarProductoCommandValidator : AbstractValidator<ActualizarProductoCommand>
    {
        public ActualizarProductoCommandValidator() 
        {
            ...
            RuleFor(x => x.Codigo)
                .MaximumLength(45).WithMessage("El código del producto no puede exceder los 45 caracteres.");
            RuleFor(x => x.PrecioVenta)
                .GreaterThan(0).WithMessage("El precio del producto debe ser mayor que cero.");
            RuleFor(x => x.Existencias)
                .GreaterThanOrEqualTo(0).WithMessage("Las existencias del producto no puede ser negativo.");
        }
    }
}
